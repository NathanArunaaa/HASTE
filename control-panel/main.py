...
class App(customtkinter.CTk):
    def __init__(self):
      ...
      # self.config(cursor="none")
        ...
        self.clear_database = customtkinter.CTkButton(self.tabview.tab("Debug"), text="Clear Database", text_color='red', width=30, command=lambda: self.send_command("CLEAR_DATABASE"))
        ...
        self.heater_off_button  = customtkinter.CTkButton(self.button_frame3, text="OFF", width=30, command=lambda: self.send_command("HEATER_OFF"))
        self.heater_off_button.grid(row=0, column=1, sticky="ew")
    ...
    def toggle_pump_B(self):
        if not self.pump_B_state:
            self.send_command("PUMP_B_ON")
            self.pump_B_state = True
        else:
            self.send_command("PUMP_B_OFF")
            self.pump_B_state = False 
